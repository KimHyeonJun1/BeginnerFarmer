...
<div class="row justify-content-center mt-5">
	<div class="w-75 comment">
		<div class="comment-title mb-2 d-flex align-items-center justify-content-between d-none h-px40">
			<label><span>댓글작성 [</span>
				   <span class="writing">0</span>
				   <span> / 200 ]</span>
			</label>
		...
		<div id="comment-input" role="button">
			<div class="form-control text-center py-4 guide">
				${ empty auth ? "댓글을 입력하려면 여기를 클릭 후 로그인하세요" : "댓글을 입력하세요" }
			</div>
		</div>
	</div>
</div>
...
<div class="row justify-content-center my-4">
	<div class="w-75" id="comment-list" data-writer="${ vo.board_writer }" data-id="${ vo.board_id }">
	</div>
</div>
...
function commentList(pageNo){
	$.ajax({
		url: "comment/list/${vo.board_id}/" + pageNo
	}).done(function(response){
...
$("#comment-input").on("click", function(){
	if( ${empty auth} ){
		...
		if( $(this).children(".guide").length > 0 ){
			$(`<textarea class="form-control h-px110"></textarea>`).appendTo( $(this) )
			.focus()
